...
class Converter(QThread, ISAT):
    ...
    def load_from_isat(self):
        ...
        # load cates
        self.message.emit(None, None, ' ' * 18 + '| Loading cats.')
        if os.path.exists(os.path.join(self.isat_json_root, 'isat.yaml')):
            ...
            cates.sort()
            self.cates = tuple(cates)
        self.message.emit(None, None, ' ' * 18 + '| Loaded.')
        return True

    def convert_to_isat(self):
        self.message.emit(None, None, ' ' * 18 + '| Start convert to ISAT.')
...
class COCOConverter(Converter, COCO):
    ...
    def coco_to_isat(self):
        self.message.emit(None, None, ' ' * 18 + '| -- COCO to ISAT --')

        # load from coco
        self.message.emit(None, None, ' ' * 18 + '| Loading from COCO.')
        try:
        ...
        self.message.emit(None, None, ' ' * 18 + '| Finished.')
        return True

    def isat_to_coco(self):
        self.message.emit(None, None, ' ' * 18 + '| -- ISAT to COCO --')

        # load from isat
        self.load_from_isat()

        # convert to coco
        self.message.emit(None, None, ' ' * 18 + '| Saving COCO json to {}.'.format('', '', self.coco_json_path))
        try:
...
class YOLOConverter(Converter, YOLO):
    ...
    def yolo_to_isat(self):
        self.message.emit(None, None, ' ' * 18 + '| -- YOLO to ISAT --')

        # load category file
        if self.yolo_category_file is not None:
            self.message.emit(None, None, ' ' * 18 + '| Loading YOLO categorys.')
            class_dict = {}
            with open(self.yolo_category_file, 'r') as f:
                lines = f.readlines()
                for index, line in enumerate(lines):
                    class_dict[index] = line.rstrip('\n')
                    self.message.emit(None, None, ' ' * 18 + '| {} : {}'.format(index, line.rstrip('\n')))
            self.message.emit(None, None, ' ' * 18 + '| Loaded.')
        else:
            class_dict = None

        # load from yolo
        self.message.emit(None, None, ' ' * 18 + '| Loading YOLO txts.')
    ...
    def isat_to_yolo(self):
        self.message.emit(None, None, ' ' * 18 + '| -- ISAT to YOLO --')
...
class LABELMEConverter(Converter, LABELME):
    ...
    def labelme_to_isat(self):
        self.message.emit(None, None, ' ' * 18 + '| -- ISAT to ISAT --')

        # load from labelme
        self.message.emit(None, None, ' ' * 18 + '| Loading from LABELME.')
...
class VOCConverter(Converter, VOC):
    ...
    def isat_to_voc(self):
        self.message.emit(None, None, ' ' * 18 + '| -- ISAT to VOC --')
...
class VOCConverterForDetection(Converter, VOCDetect):
    ...
    def isat_to_voc_for_detection(self):
        self.message.emit(None, None, ' ' * 18 + '| -- ISAT to VOC for Detection --')
...
class ConvertDialog(QtWidgets.QDialog, Ui_Dialog):
    def __init__(self, parent, mainwindow):
        super(ConvertDialog, self).__init__(parent=parent)
        self.setupUi(self)
