...
class YOLO(ISAT):
    ...
    def read_from_YOLO(self, img_root, txt_root, class_dict=None):
        ...
        if class_dict is not None:
            ...
            class_index_set = list(class_index_set)
            class_index_set.sort()
            self.cates = tuple(class_index_set)

    def save_to_YOLO(self, txt_root):
        os.makedirs(txt_root, exist_ok=True)
        self.cates_index_dict = {cat:index for index, cat in enumerate(self.cates)}
                ...
                with open(txt_path, 'w') as f:
                    ...
                    for group_index, group in enumerate(objects_groups):
                        ...
                        for cat in cats:
                            ...
                            crowds = set(crowds)
                            # 同iscrowd
                            class_index = self.cates_index_dict.get(cat)
                            segmentations = []
