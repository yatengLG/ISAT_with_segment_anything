...
class COCO(ISAT):
    ...
    def save_to_coco(self, annotation_file):
        ...
        pbar = tqdm.tqdm(enumerate(self.annos.items()))
        for file_index, (name, anno) in pbar:
            pbar.set_description('Integrate {}'.format(name))
            try:
...
if __name__ == '__main__':
    ...
    cc = COCO()
    cc.read_from_coco('../../utils/coco_tiny/coco_tiny.json')
    cc.save_to_ISAT('../../utils/coco_tiny/coco2isat')


    cc = COCO()
    cc.read_from_ISAT('../../utils/coco_tiny/coco2isat')
    cc.save_to_coco('../../utils/coco_tiny/isat2coco.json')